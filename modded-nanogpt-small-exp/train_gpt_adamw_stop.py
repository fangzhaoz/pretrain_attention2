...
class Hyperparameters:
    ...
    # optimization
    num_scheduled_iterations: int = 9000  # number of steps to complete lr and ws schedule
    num_extension_iterations: int = 23391  # number of steps to continue training at final lr and ws
...
if master_process:
    run_id = args.run_id
    os.makedirs("logs_adamw_small", exist_ok=True)
    logfile = f"logs_adamw_small/{run_id}.txt"
    print(logfile)
...
def get_lr(step: int):
    # Freeze after scheduled phase
    s = min(step, args.num_scheduled_iterations)  # clamp
    sched = args.num_scheduled_iterations
    cf = args.cooldown_frac

    keep_until = (1 - cf) * sched
    if s < keep_until:
        return 1.0
    elif s < sched:
        w = (sched - s) / (cf * sched)  # 1 -> 0
        return FINAL_LR + (1.0 - FINAL_LR) * w
    else:
        return FINAL_LR  # exact 0.1 in extension

def get_ws(step: int):
    # Freeze after scheduled phase
    s = min(step, args.num_scheduled_iterations)  # clamp
    sched = args.num_scheduled_iterations
    stages = len(args.ws_schedule)

    frac = 0.0 if sched == 0 else s / sched
    idx = min(int(frac * stages), stages - 1)
    ws_long = args.ws_schedule[idx]
    return ws_long // 2, ws_long
...
for step in range(train_steps + 1):
    ...
    if last_step:
        if master_process and args.save_checkpoint:
            ...
            os.makedirs(f"logs_adamw_small/{run_id}.txt", exist_ok=True)
            torch.save(log, f"logs_adamw_small/{run_id}.txt/state_step{step:06d}.pt")
...
if master_process:
    ...
    out_dir = f"logs_adamw_small/{run_id}"
