...
if master_process:
    ...
    os.makedirs("logs_adamw_long", exist_ok=True)
    logfile = f"logs_adamw_long/muon_5960.txt"
    print(logfile)
...
# optimizer2 = torch.optim.AdamW(hidden_matrix_params, lr=8e-4,  betas=(0.85, 0.85), eps=1e-10, weight_decay=float(args2.wd), fused=True)
...
for step in range(train_steps + 1):
    ...
    if last_step:
        if master_process and args.save_checkpoint:
            ...
            os.makedirs(f"logs_adamw_long/muon_5960", exist_ok=True)
            torch.save(log, f"logs_adamw_long/muon_5960/state_step{step:06d}.pt")
...
if master_process:
    ...
    # Build a fast membership set for the exact Parameter objects we care about
    hidden_ids = {id(p) for p in hidden_matrix_params}

    # Helper to build clean names like layer5_q, layer5_o, layer5_mlp_fc, layer5_mlp_proj
    def pretty_name(full_name: str, p: torch.Tensor, idx: int | None = None) -> str:
        # Expect names like: blocks.<L>.(attn|mlp).<param>
        m = re.match(r"^blocks\.(\d+)\.(attn|mlp)\.(\w+)$", full_name)
        if not m:
            # Fallback to the raw name (rare)
            return full_name if idx is None else f"{full_name}_{idx}"
        layer, submod, pname = m.groups()
        if submod == "attn" and pname == "qkvo_w":
            # Map slice index -> q/k/v/o tag
            tag = ["q", "k", "v", "o"][idx]
            return f"layer{layer}_{tag}"
        if submod == "mlp" and pname == "fc_w":
            return f"layer{layer}_mlp_fc"
        if submod == "mlp" and pname == "proj_w":
            return f"layer{layer}_mlp_proj"
        # Generic fallback
        return f"layer{layer}_{submod}_{pname}" if idx is None else f"layer{layer}_{submod}_{pname}_{idx}"

    with torch.no_grad():
        # Ensure latest params are on CPU for SVD to avoid large GPU mem spikes
        for full_name, p in model.named_parameters():
            if id(p) not in hidden_ids:
                continue
            # Handle shapes:
            # - 2D: standard weight matrix -> svdvals(W)
            # - 3D qkvo (4, hdim, dim): split along first axis and name q/k/v/o separately
            if p.ndim == 2:
                W = p.detach().to(dtype=torch.float32, device="cpu")
                s = torch.linalg.svdvals(W)  # 1D tensor of singular values
                sv_out[pretty_name(full_name, p)] = s
            elif p.ndim == 3 and p.size(0) == 4 and full_name.endswith("qkvo_w"):
                # Split into q,k,v,o
                W = p.detach().to(dtype=torch.float32, device="cpu")
                for i in range(4):
                    s = torch.linalg.svdvals(W[i])
                    sv_out[pretty_name(full_name, p, idx=i)] = s
            else:
                # If any unexpected >2D weight appears, flatten to (out, in) conservatively
                W = p.detach().to(dtype=torch.float32, device="cpu")
                out_dim = W.shape[-2]
                in_dim  = W.shape[-1]
    ...
    # Save with pickle (mapping: name -> torch.Tensor of singular values)
    os.makedirs("logs_adamw_long/muon_5960", exist_ok=True)
    sv_path = "logs_adamw_long/muon_5960/singular_values.pkl"
