...
if master_process:
    ...
    os.makedirs("logs_spectral", exist_ok=True)
    logfile = f"logs_spectral/{run_id_full}.txt"
    print(logfile)
...
optimizer2 = torch.optim.AdamW(hidden_matrix_params, lr=8e-4,  betas=(0.85, 0.85), eps=1e-10, weight_decay=8e-1, fused=True)
...
for step in range(train_steps + 1):
    ...
    if last_step:
        if master_process and args.save_checkpoint:
            ...
            os.makedirs(f"logs_spectral/{run_id_full}", exist_ok=True)
            torch.save(log, f"logs_spectral/{run_id_full}/state_step{step:06d}.pt")
...
if master_process:
    ...
    os.makedirs("logs_spectral/adamw_10000", exist_ok=True)
    sv_path = "logs_spectral/adamw_10000/singular_values.pkl"
