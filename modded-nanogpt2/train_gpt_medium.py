...
class AdamWExplicit(torch.optim.Optimizer):
    r"""AdamW with an optional custom *decoupled* weight-decay stage.

    Group keys (like torch.optim.AdamW + wd controls):
      lr, betas, eps, weight_decay, amsgrad, maximize,
      wd_mode: 'custom' | 'decoupled' | 'none'
      wd_fn: Optional[callable(p, state, group)]    # used iff wd_mode == 'custom'
    """
    def __init__(
        self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
        weight_decay=0.0, amsgrad=False, maximize=False,
    ):
        if lr < 0.0: raise ValueError("Invalid lr")
        if eps < 0.0: raise ValueError("Invalid eps")
        b1, b2 = betas
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if len(state) == 0:
                    ...
                    if amsgrad:
                        state["max_exp_avg_sq"] = torch.zeros_like(p, dtype=torch.float32, device=p.device)

                state["step"] += 1

                # ---- (A) Decoupled WD stage (like AdamW) ----
                p.data.add_(p.data, alpha=-lr * wd)

                # ---- (B) Adam update (FP32) ----
                exp_avg = state["exp_avg"]; exp_avg_sq = state["exp_avg_sq"]
                g32 = grad.detach().to(torch.float32)
                ...
                p32.addcdiv_(exp_avg, denom, value=-step_size)
                p.data.copy_(p32.to(dtype=p.dtype))
...
def update(acc_bf16_view_u16: Tensor, mantissa: Tensor, momentum_buffer: Tensor, grad: Tensor, momentum: Tensor, eff_lr: Tensor, eff_weight_decay: Tensor, 
    ...
    # # ---------- vanilla weight decay ----------
    # if not bool(use_l1_cwd.to("cpu").item()):
    #     acc_m_u32.view(torch.float32).mul_(1 - eff_weight_decay)

    # # ---------- L1-CWD decay ----------
    # if bool(use_l1_cwd.to("cpu").item()):
    #     x_fp = acc_m_u32.view(torch.float32) 
    #     mask = (x_fp * v) > 0 
    #     x_fp[mask].mul_(1.0 - eff_weight_decay)


    # ---------- L1/L2 hybrid CWD (mask + small-dot quadratic) ----------
    x_fp = acc_m_u32.view(torch.float32)

    # choose your epsilon (scalar tensor on the same device/dtype)
    # you can tune this; e.g., fixed value or tie to lr (e.g., 0.1*eff_lr)
    cwd_eps = torch.as_tensor(1e-3, dtype=x_fp.dtype, device=x_fp.device)

    # elementwise dot with update direction
    xu = x_fp * v

    # masks
    M1 = xu.ge(0)                 # 1{x*u >= 0}
    M2 = xu.abs().le(cwd_eps)     # 1{|x*u| <= eps}

    # build Δ = 1{x*u>=0} * x  +  0.5 * 1{|x*u|<=eps} * x^2
    m1 = M1.to(x_fp.dtype)
    m2 = M2.to(x_fp.dtype)

    delta = x_fp * m1
    delta2 = 0.5 * (x_fp * x_fp) * m2
    delta = delta + delta2

    # decoupled update: x <- x - eff_weight_decay * Δ
    x_fp.add_(delta, alpha=-eff_weight_decay)
    # ---------- end hybrid CWD ----------
    ...
    # # ---------- decoupled WD + op CWD ----------
    # x_fp = acc_m_u32.view(torch.float32)
    # # x_fp.mul_(1 - eff_weight_decay) # accumulative
    # eps = x_fp.new_tensor(1e-12)
    ...
    # # Apply decay: -ηλ * d_spec  (eff_weight_decay already equals ηλ)
    # # x_fp.mul_(1 - eff_weight_decay) # separate
    # x_fp.add_(d_spec, alpha=-eff_weight_decay) 


    # # ---------- Nuclear-norm WD (SVT after Muon step; robust SVD) ----------
    # x_fp = acc_m_u32.view(torch.float32)
    # G_star = zeropower_via_newtonschulz5(x_fp.bfloat16()).to(dtype=x_fp.dtype)
    # x_fp.add_(G_star, alpha=-eff_weight_decay) 

    # # ---------- x^4 ----------
    # x_fp = acc_m_u32.view(torch.float32)
    # x_fp.add_(4*x_fp**3, alpha=-eff_weight_decay) 


    # # ---------- decoupled L1 WD (proximal ℓ1: soft-threshold) ----------
    # x_fp = acc_m_u32.view(torch.float32)
    # absx = x_fp.abs()
    # x_fp = x_fp.sign() * torch.clamp(absx - eff_weight_decay, min=0.0)

    # # ---------- Elastic-Schatten: L2 + tiny spectral soft-cap on σ₁ ----------

    # # (A) L2 (Frobenius) decoupled shrink — your current baseline
    # x_fp = acc_m_u32.view(torch.float32)
    # x_fp.mul_(1.0 - eff_weight_decay)

    # # (B) Tiny spectral soft-cap (no SVD; 2 power iterations)
    # if x_fp.ndim >= 2:
    #     # Tunables (start tiny)
    #     spec_cap_frac   = 0.10    # relative to eff_weight_decay
    #     spec_cap_target = 1.25    # target ≈ 1.25 × avg singular value
    #     pow_iters       = 2

    #     eps = x_fp.new_tensor(1e-12)
    #     m, n = x_fp.shape[-2], x_fp.shape[-1]

    #     if x_fp.ndim == 2:
    #         # Power iteration for top-1 (a, b, σ)
    #         b = x_fp.new_ones(n)
    #         b = b / (b.norm() + eps)
    #         for _ in range(pow_iters):
    #             a = (x_fp @ b);         a = a / (a.norm() + eps)
    #             b = (x_fp.mT @ a);      b = b / (b.norm() + eps)
    #         # σ ≈ Rayleigh; outer = a b^T
    #         sigma = (a * (x_fp @ b)).sum().abs().clamp_min(eps)
    #         outer_ab = a[:, None] * b[None, :]

    #         # Cap relative to avg σ (≈ ||X||_F / sqrt(min(m,n)))
    #         avg_sigma = x_fp.norm() / (float(min(m, n)) ** 0.5)
    #         tau = spec_cap_target * avg_sigma
    #         cap = (sigma - tau).clamp_min(0.0)  # only push when σ₁ > τ

    #         spec_coeff = spec_cap_frac * eff_weight_decay
    #         # grad ~ 2*(σ-τ) * a b^T  (quadratic cap)
    #         x_fp.add_(outer_ab, alpha=-(2.0 * spec_coeff * cap))

    #     else:
    #         # Batched version
    #         b = x_fp.new_ones(x_fp.shape[0], n)
    #         b = b / (b.norm(dim=-1, keepdim=True) + eps)
    #         for _ in range(pow_iters):
    #             a = torch.matmul(x_fp, b[..., None]).squeeze(-1)
    #             a = a / (a.norm(dim=-1, keepdim=True) + eps)
    #             b = torch.matmul(x_fp.transpose(-2, -1), a[..., None]).squeeze(-1)
    #             b = b / (b.norm(dim=-1, keepdim=True) + eps)

    #         Xb = torch.matmul(x_fp, b[..., None]).squeeze(-1)
    #         sigma = (a * Xb).sum(dim=-1).abs().clamp_min(eps)  # (B,)
    #         outer_ab = a[..., :, None] * b[..., None, :]       # (B,m,n)

    #         avg_sigma = x_fp.norm(dim=(-2, -1)) / (float(min(m, n)) ** 0.5)  # (B,)
    #         tau = spec_cap_target * avg_sigma
    #         cap = (sigma - tau).clamp_min(0.0)                                 # (B,)

    #         spec_coeff = spec_cap_frac * eff_weight_decay
    #         x_fp.add_(outer_ab, alpha=-(2.0 * spec_coeff * cap)[..., None, None])

    # # ---------- end Elastic-Schatten ----------


    # # ---------- Schatten-p (p=1.5) using top-1 mode only ----------

    # x_fp = acc_m_u32.view(torch.float32)

    # # Keep (or skip) L2 if you want this to be purely p-norm:
    # # x_fp.mul_(1.0 - eff_weight_decay)

    # if x_fp.ndim >= 2:
    #     # Tunables
    #     p_val     = 1.5
    #     sp15_frac = 0.05   # relative to eff_weight_decay
    #     pow_iters = 2
    ...
    #     if x_fp.ndim == 2:
    #         # Top-1 (a, b, σ) by power iteration
    #         b = x_fp.new_ones(n)
    #         b = b / (b.norm() + eps)
    #         for _ in range(pow_iters):
    #             a = (x_fp @ b);         a = a / (a.norm() + eps)
    ...
    #         Xb = (x_fp @ b)
    #         sigma = (a * Xb).sum().abs().clamp_min(eps)        # scalar
    #         outer_ab = a[:, None] * b[None, :]

    #         # ∂/∂X [λ ∑ σ_i^p] ≈ λ * p * σ₁^{p-1} * a b^T
    #         sp_coeff = sp15_frac * eff_weight_decay
    #         gain = p_val * (sigma ** (p_val - 1.0))
    #         x_fp.add_(outer_ab, alpha=-(sp_coeff * gain))
    ...
    #         sp_coeff = sp15_frac * eff_weight_decay
    #         gain = p_val * (sigma ** (p_val - 1.0))             # (B,)
    #         x_fp.add_(outer_ab, alpha=-(sp_coeff * gain)[..., None, None])

    # # ---------- end Schatten-p (p=1.5) ----------

    # # ---------- L1-CWD on top of Muon + decoupled L2 WD (standard; fallback on non-finite) ----------
    # x_fp = acc_m_u32.view(torch.float32)

    # # 2) L1-CWD with power iteration (no guards; fallback if payload non-finite)
    # M_l1 = (x_fp * v).ge(0)  # 1{X ⊙ v >= 0}

    # x_fp.mul_(1.0 - eff_weight_decay)
    # x_fp.add_(v, alpha=-eff_lr)

    # # k = eff_weight_decay * x_fp  + eff_lr * v
    ...
    #     if x_fp.ndim == 2:
    #         m, n = x_fp.shape
    #         b = x_fp.new_ones(n); b = b / b.norm()          # no guards by request
    #         for _ in range(pow_iters):
    #             a = (x_fp @ b);    a = a / a.norm()
    #             b = (x_fp.mT @ a); b = b / b.norm()

    #         uvT = a[:, None] * b[None, :]                   # (m,n)
    #         if bool(torch.isfinite(uvT).all().to("cpu").item()):
    #             M_sp  = (x_fp * uvT).ge(0)
    #             S     = M_l1 & M_sp
    #             Delta = torch.where(S, x_fp, torch.zeros_like(x_fp))
    #             x_fp.add_(Delta, alpha=-eff_weight_decay)
    #         else:
    #             _log_fallback("[L1-CWD fallback] non-finite (a b^T) from power iteration; keeping Muon+L2")

    #     else:
    #         # batched (B,m,n)
    #         B, m, n = x_fp.shape
    #         b = x_fp.new_ones(B, n); b = b / b.norm(dim=-1, keepdim=True)
    #         for _ in range(pow_iters):
    #             a = torch.matmul(x_fp, b[..., None]).squeeze(-1);               a = a / a.norm(dim=-1, keepdim=True)
    #             b = torch.matmul(x_fp.transpose(-2, -1), a[..., None]).squeeze(-1); b = b / b.norm(dim=-1, keepdim=True)

    #         uvT = a[..., :, None] * b[..., None, :]        # (B,m,n)
    #         if bool(torch.isfinite(uvT).all().to("cpu").item()):
    #             M_sp  = (x_fp * uvT).ge(0)
    #             S     = M_l1 & M_sp
    #             Delta = torch.where(S, x_fp, torch.zeros_like(x_fp))
    #             x_fp.add_(Delta, alpha=-eff_weight_decay)
    #         else:
    #             _log_fallback("[L1-CWD fallback] non-finite (a b^T) from batched power iteration; keeping Muon+L2")
    # # else: vectors/scalars → keep Muon+L2 only
    # # x_fp.add_(k, alpha=-1.0)

    # # pack back to bf16 halves
    # x_i32  = x_fp.view(torch.int32)
    # mask16 = x_fp.new_tensor(0xFFFF, dtype=torch.int32)
    # lo_u16 = (x_i32 & mask16).to(torch.uint16).contiguous()
    # hi_u16 = (torch.bitwise_right_shift(x_i32, 16) & mask16).to(torch.uint16).contiguous()
    # acc_bf16_view_u16.copy_(hi_u16)
    # mantissa.copy_(lo_u16)
    # # ---------- end L1-CWD block ----------


    # # ---------- L2-CWD via two projections (spectral half-space, then orth-to-v) ----------
    # # Current param (if you prefer a standalone tensor, replace with the packed->fp32 pattern)
    # x_fp = acc_m_u32.view(torch.float32)

    # # Base: decoupled L2 then Muon step (same order you used in L1-CWD PI version)
    # k = eff_weight_decay * x_fp  + eff_lr * v
    # # x_fp.mul_(1.0 - eff_weight_decay)
    # # x_fp.add_(v, alpha=-eff_lr)
    ...
    #     if x_fp.ndim == 2:
    #         m, n = x_fp.shape
    #         # --- power iteration for top-1 (a, b). No guards by request ---
    #         b = x_fp.new_ones(n); b = b / b.norm()
    #         for _ in range(pow_iters):
    #             a = (x_fp @ b);    a = a / a.norm()
    #             b = (x_fp.mT @ a); b = b / b.norm()
    ...
    #         if bool(torch.isfinite(uvT).all().to("cpu").item()):
    #             # 1) Spectral projection onto half-space <X,Q> >= 0
    #             dot_xu    = (x_fp * uvT).sum().clamp_max(0.0)          
    #             dot_uu  = (uvT * uvT).sum().clamp_min(eps)            
    #             Xsafe = x_fp - (dot_xu / dot_uu) * uvT

    #             # 2) Remove component along v: (I - Π_v) Xsafe
    #             dot_xu = (Xsafe * v).sum().clamp_max(0.0)        
    #             dot_uu = (v * v).sum().clamp_min(eps)  
    #             proj_coeff = dot_xu / dot_uu
    #             proj_on_v  = proj_coeff * v
    #             X_perp_v   = Xsafe - proj_on_v

    #             # Decoupled L2-CWD shrink
    #             x_fp.add_(X_perp_v, alpha=-eff_weight_decay)
    #         else:
    #             _log_fallback("[L2-CWD fallback] non-finite spectral payload Q from power iter; keeping Muon+L2")
    ...
    #         if bool(torch.isfinite(uvT).all().to("cpu").item()):
    #             # 1) Spectral projection per slice
    #             dot_xu    = (x_fp * uvT).sum(dim=(-2, -1)).clamp_max(0.0)              
    #             dot_uu  = (uvT * uvT).sum(dim=(-2, -1)).clamp_min(eps)     
    #             Xsafe = x_fp - (dot_xu / dot_uu)[..., None, None] * uvT

    #             # 2) Remove component along v per slice
    #             dot_xu = (Xsafe * v).sum(dim=(-2, -1)).clamp_max(0.0)          
    #             dot_uu = (v * v).sum(dim=(-2, -1)).clamp_min(eps)                 
    #             proj_coeff = (dot_xu / dot_uu)[..., None, None]  
    #             proj_on_v  = proj_coeff * v
    #             X_perp_v   = Xsafe - proj_on_v

    #             x_fp.add_(X_perp_v, alpha=-eff_weight_decay)
    #         else:
    #             _log_fallback("[L2-CWD fallback] non-finite spectral payload Q (batched) from power iter; keeping Muon+L2")
    # # else: vectors/scalars → keep Muon+L2 only
...
class Muon(torch.optim.Optimizer):
    ...
    def __init__(self, params, lr=0.02, weight_decay=0.01, momentum=0.95, rank=0, world_size=1):
        ...
        # change here
        # initialize a checkpointable counter in param_groups[0]
        if len(self.param_groups) == 0:
            self.add_param_group({'params': []})
        self.param_groups[0].setdefault('_cwd_step_idx', 0)
        self.step_idx = int(self.param_groups[0]['_cwd_step_idx'])

        # switch point
        self.l1_cwd_until = 4500  # inclusive of steps 0..4999
    ...
    def step(self):
        ...
        #                 use_l1_cwd=use_l1_flag,  # <- the switch
        #             )
        #         futures.append(dist.all_gather(params_pad[base_i:base_i + self.world_size], params_pad[base_i + self.rank], async_op=True).get_future())
        # torch.futures.collect_all(futures).wait()

        # # advance and persist for checkpointing
        # self.step_idx += 1
        # self.param_groups[0]['_cwd_step_idx'] = int(self.step_idx)

        # change here
        # refresh from param_groups in case we loaded a checkpoint
        self.step_idx = int(self.param_groups[0].get('_cwd_step_idx', 0))

        # if self.step_idx == self.l1_cwd_until and self.rank == 0:
        #     print("[WD switch] step 4500: switching to default decoupled L2 WD")

        # compute the per-step flag once per rank
        use_l1 = self.step_idx < self.l1_cwd_until
        use_l1_flag = None  # will materialize once on device as 0-dim bool tensor

        futures: list[torch.Future] = []
        for group in self.param_groups:
            params: list[Tensor] = group["params"]

            # materialize the flag tensor on the correct device *once*
            if use_l1_flag is None:
                use_l1_flag = torch.tensor(use_l1, dtype=torch.bool, device=params[0].device)
...
optimizer2 = torch.optim.AdamW(hidden_matrix_params, lr=8e-3,  betas=(0.8, 0.95), eps=1e-10, weight_decay=0.0, fused=True)
